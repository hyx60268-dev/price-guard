...
export async function yahooCompare(page,item,settings){
  ...
  return {query,searchUrl:url,lowestPrice:lowest?.price??null,lowestUrl:lowest?.url??'',recommendedPrice:recommended,candidates:candidates.slice(0,5),cardCount,status:candidates.length?'ok':cardCount?'manual_review':'page_empty'};
}
