...
export async function xianyuCost(page,item,settings){
  ...
  const loginVisible=await page.getByText('登录',{exact:true}).count().catch(()=>0);
  if(!cardCount && (loginVisible || /login|signin/i.test(page.url()))) return {query,searchUrl:url,status:'login_required',samples:[],averageCNY:null,cardCount};
  ...
  return {query,searchUrl:url,status:selected.length?'ok':cardCount?'manual_review':'page_empty',samples:selected,averageCNY:average(selected.map(x=>x.price)),cardCount};
}
